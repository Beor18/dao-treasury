...
export function TreasuryManager({ requests }: TreasuryManagerProps) {
  const [activeRequest, setActiveRequest] = React.useState<Request | null>(null)
    ...
    <div className="grid gap-4 md:grid-cols-2">
            ...
            {requests.map((request) => (
              ...
              >
                <div className="flex justify-between items-center w-full">
                  <span>Request #{request.id} - {request.amount.toLocaleString()} tokens</span>
                  <Badge variant={request.status === 'approved' ? 'success' : request.status === 'denied' ? 'destructive' : 'secondary'}>
                    {request.status}
                  </Badge>
                </div>
              </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Review and make decisions on fund requests</CardDescription>
        </CardHeader>
        <CardContent>
          {activeRequest ? (
            <div className="space-y-4">
              ...
              <div>
                <strong>Amount:</strong> {activeRequest.amount.toLocaleString()} tokens
              </div>
              <div>
